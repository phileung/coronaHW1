...
background2.x = background.x+3599;
...
for i=1,craternum do
	...
	crater.x = -1*(background.x + 100 + i*math.random(450,1250));
...
local function onCollision(self, event)
	...
	if (self.name == "car" and event.other.name == "obstacle") then
		carDied = true;
 		local tm = timer.performWithDelay(200, removeObj, 1)
...
function fireshooot(event)
	if (car ~= nil) and (car.x ~= nil) then
		local bullet = display.newRect(car.x+150, car.y,10,30);
		bullet.name = "flybullet";
		bullet.gravityScale = 0;
		physics.addBody(bullet,"static",{friction=0});
